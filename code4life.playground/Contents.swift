...
struct SampleData {
    ...
    init(_ inputs: [String]) {
        ...
        carriedBy = SampleLocation(rawValue: Int(inputs[1])!)! // 0 = me, 1 = other, -1 = cloud ... only one can cerry a sample
...
func collectSamples() {
    ...
    // if sampleDatas.lazy.filter({$0.carriedBy == .player}).count == 2 {
    //rank = max(1, rank - 1)
    //  }
...
func needsCollectSamples() -> Bool {
    let carryingSamples = sampleDatas.lazy.filter({ $0.carriedBy == .player })
...
func needsToSendSampleToCloud() -> Bool {
    let samples = sampleDatas.lazy.filter({ $0.carriedBy == .player && $0.isAnalyzed() })
...
func sendSampleToCloud() {
    ...
    guard let sample = sampleDatas.first(where: { $0.carriedBy == .player }) else {
...
func firstUsefulCloudSample() -> SampleData? {
    ...
    let cloudSamples = sampleDatas.lazy.filter({ sample in
        if sample.carriedBy == .cloud && sample.isAnalyzed() {
...
func firstAnalyzeableSample() -> SampleData? {
    return sampleDatas.lazy.first(where: {$0.carriedBy == .player && !$0.isAnalyzed()})
}
...
func moleculeToGet(from sampleLocation: SampleLocation) -> String? {
    let samples = sampleDatas.lazy.filter({$0.carriedBy == sampleLocation && $0.isAnalyzed()}).sorted(by: {$0.health > $1.health})

    var storedMolecules: MoleculesStack
    var expertise: MoleculesStack

    switch sampleLocation {
    case .player:
        storedMolecules = player.stored
        expertise = player.expertise
    default:
        storedMolecules = enemy.stored
        expertise = enemy.expertise
    }
...
func thiefModeMolecule() -> String? {
    ...
    if player.stored.sum() < 10 {
        return moleculeToGet(from: .enemy)
    }
    return nil
}
...
func gatherMolecules() {
    ...
    if let molecule = moleculeToGet(from: .player) {
        print("CONNECT \(molecule)")
        return
    }

    if let molecule = thiefModeMolecule() {
        print("CONNECT \(molecule)")
        return
    }
...
func needsToGatherMolecules() -> Bool {
    ...
    if moleculeToGet(from: .player) != nil {
        return true
    }

    return moleculeToGet(from: .enemy) != nil
}
...
func getFirstPeoduceableSample() -> SampleData? {
    let sample = sampleDatas.first(where: { $0.carriedBy == .player && $0.health != -1 && player.effectiveAmountOfMolecules().contains($0.costs)})
...
while mode == .GAME {
    ...
    let samples = sampleDatas.lazy.filter({$0.carriedBy == .player}).sorted(by: {$0.health > $1.health})
    for sample in samples {
        debug("\(sample)")
    }
    debug("\(projects)")
    let samplesOther = sampleDatas.lazy.filter({$0.carriedBy == .enemy}).sorted(by: {$0.health > $1.health})
